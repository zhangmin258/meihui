...
//					console.log(this.idArrsort);
				...
				for(var i=0;i<this.priceList.length;i++){    //ids排序
					...
					if(this.priceList[i].ids.toString()==this.idArrsort.toString()){   //一旦匹配立马跳出
						...
						this.proData.member_price=this.priceList[i].member_price;
		...
		buyProSub:function(){
			...
			if(flag1==true){
				...
				this.$http({method:'POST',url:URL+'/person/Order/sumbit_order',data:{g_id:this.proId,a_id:this.attrId,uid:localStorage.uid,num:1}}).then(
					function(response){
...
//								console.log(response.data);
		...
		collectPro:function(){
			if(localStorage.uid=='' || localStorage.uid==undefined || localStorage.uid==null){
		        var btnArray = ['取消', '确认'];
		        mui.confirm('登录后才能收藏！', '确认框', btnArray, function(e) {
		            if (e.index == 1){
						jump('../land/login.html');			
		            } else {} 
		        });			
			}else{
				if(this.collect==true){      //已经收藏-取消收藏
					this.$http({method:'POST',url:URL+'/shopping/Goods/collection',data:{g_id:this.proId,uid:localStorage.uid,type:0}}).then(
						function(response){
							if(response.data.success==true){
								this.collect=false;
								mui.toast('你已取消收藏该商品');
							}
						},function(response){mui.toast('取消收藏失败，请检查网络！');}
					);					
				}else if(this.collect==false){   //未收藏-立即收藏 
					this.$http({method:'POST',url:URL+'/shopping/Goods/collection',data:{g_id:this.proId,uid:localStorage.uid,type:1}}).then(
						function(response){
							if(response.data.success==true){
								this.collect=true;
								mui.toast('你已成功收藏该商品');
							}
						},function(response){mui.toast('收藏失败，请检查网络！');}
					);					
				}
			}			
		},
		...
		joinCarSub:function(){
			...
			if(flag1==true){
				mui('.mnhcar').button('loading');
				this.$http({method:'POST',url:URL+'/person/Car/join_car',data:{g_id:this.proId,uid:localStorage.uid,a_id:this.attrId,num:1}}).then(
					function(response){
//							console.log(response.data);
						if(response.data.success==true){
							mui('.mnhcar').button('reset');
							mui.toast('添加成功！可到购物车中查看');
						}
					},function(response){mui.toast('添加失败,请检查网络！');mui('.mnhcar').button('reset');}
		...
		joinCar:function(){
			if(localStorage.uid=='' || localStorage.uid==undefined || localStorage.uid==null){
		        ...
		        mui.confirm('你还没有登录哦！', '确认框', btnArray, function(e) {
		       	...
		       	var btnArray = ['暂不考虑', '我要优惠'];
		        mui.confirm('成为会员，享受更多优惠！', '确认框', btnArray, function(e) {
		            if (e.index == 1){
						jump('../myown/attestation.html');
		            } else {
		            	this.joinCarSub();
