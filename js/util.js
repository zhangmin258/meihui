var util = {
	options: {
		ACTIVE_COLOR: "#ff6699",
		NORMAL_COLOR: "#999999",
		subpages:['html/mnhshophome.html', 'html/tab-webview-subpage-contact.html']
	},
	/**
	 *  简单封装了绘制原生view控件的方法
	...
	toggleNview: function(currIndex) {
		...
		util.updateSubNView(currIndex, util.options.ACTIVE_COLOR,currIndex);//（图标选中id，选中颜色，初始id）
		util.updateSubNView(currIndex + 1, util.options.ACTIVE_COLOR,currIndex);//（文字选中id，选中颜色，初始id）
		// 重绘兄弟tag 反之排除当前点击的icon和text 循环
		for(var i = 0; i < 8; i++) {
			if(i !== currIndex && i !== currIndex + 1) {
				util.updateSubNView(i, util.options.NORMAL_COLOR,currIndex);//（其他选中id，选中颜色，初始id）
			}
		}
	},	
	/*
	 ...
	 */
	updateSubNView: function(currIndex, color,Index) {		
...
//			非选中回调函数
			nviewEvent.drawText(currTag.textes, currTag.position, util.changeColor(currTag.textStyles, color), currTag.id);
//			选中的图标内容回调函数
		if (currIndex !== Index && currIndex !== Index+1) {			
