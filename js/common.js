...
//targetTouches:      //当前dom元素上手指的列表，尽量使用这个代替touches,可以防止事件污染
//event.touches.length==1/2   //1表示一根手指 2两根。。。。

//offsetLeft: 计算属性是数字           style.left:是赋值属性 数字+px 

function drag(nodeid,parentNodeid){
...
//  			var pWidth = parentNode.innerWidth,pHeight = parentNode.innerHeight;  
	...
	obj.addEventListener('touchstart',function(event){   
		...
		obj.addEventListener('touchmove',function(event){ 
			...
			//右边
			if(obj.offsetLeft>pWidth-obj.offsetWidth){obj.style.left=pWidth-obj.offsetWidth+'px'};
			//上边
			if(obj.offsetTop<0){obj.style.top=0};
			//下边
			if(obj.offsetTop>pHeight-obj.offsetHeight){obj.style.top=pHeight-obj.offsetHeight+'px'};
		});
    
    });
    
// element.removeEventListener(event, function, useCapture) event: 要移除的事件名称，必须  /function：移除的函数，必须 /useCapture：移除的阶段，可选(捕获true/冒泡false)
...
//数组分割
				//要分割的数组，拆分的每个数组的长度 
//function cutArr (arr,length){
//	var result=[];
//	for(var i=0;i<arr.length;i+=length){  //如:length=3时，i=0,36,9...; 
